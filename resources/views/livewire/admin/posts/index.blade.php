    ...
    <div class="mb-3 bg-white px-2 py-1">
        ...
        <x-table :headers="$headers" :rows="$all">
            ...
            @scope('cell_avatar', $post)
                <img class="w-12 rounded" src="{{ 'https://i0.wp.com/thewordwarrior.com/wp-content/uploads/woocommerce-placeholder.png?fit=655,655&ssl=1' ?? Storage::url($post->avatar->path ?? 'photos/sample_post.webp') }}" alt="">
            @endscope
